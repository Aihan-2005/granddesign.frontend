...
import { useCartStore } from "@/zustand/cartStore";
import { motion, AnimatePresence } from "framer-motion";
import { useWishlistStore } from "@/zustand/favoritesStore"; // تغییر مسیر به wishlistStore
...
const ProductCard = ({ product }) => {
  const { addItem, items } = useCartStore();
  const { toggleWishlistItem, wishlistItems } = useWishlistStore(); // استخراج توابع و state از store
  ...
  return (
    <motion.div 
      className="border rounded p-4 shadow hover:shadow-lg transition relative" // اضافه کردن relative برای موقعیت دهی مطلق دکمه قلب
      whileHover={{ y: -5 }}
    >
      {/* دکمه قلب در بالای کارت محصول */}
      <motion.button
        onClick={() => toggleWishlistItem(product)} // استفاده از toggleWishlistItem به جای toggleItem
        className="absolute top-3 left-3 z-10 p-2 bg-white rounded-full shadow-md"
        whileTap={{ scale: 0.9 }}
        aria-label={isFavorite ? "حذف از علاقه‌مندی‌ها" : "افزودن به علاقه‌مندی‌ها"}
      >
