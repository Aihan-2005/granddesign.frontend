...
export default function SendOtp(){
  ...
  const {setloginAndRegister ,loginAndRegister , setInformationUser , informationUser}=useLoginStore()
  const [information , setInformation] = useState({})


   async function callRegister(obj) {
    setInformationUser(obj)
     const res = await registerFunction(informationUser)
     if (res.success){
router("/")
     }
  }
      ...
      <div className="w-full max-w-sm flex flex-col items-center space-y-8">
        ...
        <div className="w-full space-y-6">
          <div className="relative">
            <input
            value={information.otp}
              type="text"
              onChange={(event)=>{
                setInformation(prev=>{return {...prev , otp:event.target.value}})
              }}
              placeholder="otp"
