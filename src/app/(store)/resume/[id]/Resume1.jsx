...
const GalleryItem = memo(({ item, onClick, index }) => {
  ...
  return (
    <div
      ref={ref}
      className="group relative aspect-[4/3] rounded-xl overflow-hidden cursor-pointer bg-gray-800"
      onClick={() => onClick(item)}
    >
      {!inView && (
      ...
      {(inView || isEager) && (
        <Image
          ...
          quality={60}
          placeholder="blur"
          blurDataURL="data:image/jpeg;base64,"
        />
      )}
  ...
  return (
    <AnimatePresence>
      ...
      >
        <button
          onClick={onClose}
          className="absolute top-5 right-5 w-10 h-10 flex items-center justify-center rounded-full bg-white/10 text-white text-xl hover:bg-white/20 transition z-10"
          aria-label="بستن"
        >
          ✕
        </button>
        ...
        <motion.div
          ...
          className="relative w-[92vw] md:w-[72vw] h-[82vh]"
          onClick={(e) => e.stopPropagation()}
        >
          <Image
            src={activeImage.src}
